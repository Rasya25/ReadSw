...
import baileys, { jidNormalizedUser, extractMessageContent, areJidsSameUser, downloadMediaMessage } from '@chocoopy/baileys';
...
export function Client({ sockk, store }) {
	const client = Object.defineProperties(sockk, {
		getName: {
			value(jid) {
		...
		sendContact: {
			async value(jid, number, quoted, options = {}) {
				...
				for (let v of number) {
					...
					v = v.replace(/\D+/g, '');
					list.push({
						displayName: sockk.getName(v + '@s.whatsapp.net'),
						vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${sockk.getName(v + '@s.whatsapp.net')}\nFN:${sockk.getName(v + '@s.whatsapp.net')}\nitem1.TEL;waid=${v}:${v}\nEND:VCARD`,
					});
				}
				return sockk.sendMessage(
					jid,
					{
						contacts: {
		...
		downloadMediaMessage: {
			async value(message, filename) {
				let media = await downloadMediaMessage(
					...
					{},
					{
						logger: pino({ timestamp: () => `,"time":"${new Date().toJSON()}"`, level: 'fatal' }).child({ class: 'sockk' }),
						reuploadRequest: sockk.updateMediaMessage,
					}
				);
		...
		cMod: {
			value(jid, copy, text = '', sender = sockk.user.id, options = {}) {
				...
				copy.key.fromMe = areJidsSameUser(sender, sockk.user.id);
...
export default async function serialize(sockk, msg, store) {
	const m = {};
	...
	if (msg.key) {
		...
		m.sender = jidNormalizedUser(m.fromMe ? sockk.user.id : m.isGroup ? m.participant : m.from);
	}

	if (m.isGroup) {
		if (!(m.from in store.groupMetadata)) store.groupMetadata[m.from] = await sockk.groupMetadata(m.from);
		...
		m.isBotAdmin = m.isGroup && !!m.groupAdmins.find(member => member.id === jidNormalizedUser(sockk.user.id));
	}
	...
	if (m.message) {
		...
		m.prefix = '^'; //new RegExp('^[^]', 'gi').test(m.body) ? m.body.match(new RegExp('^[^]', 'gi'))[0] : '';
		...
		if (m.msg?.contextInfo?.quotedMessage) {
			...
			if (m.quoted.message) {
				...
				m.quoted.key = {
					...
					fromMe: areJidsSameUser(jidNormalizedUser(m.msg?.contextInfo?.participant), jidNormalizedUser(sockk?.user?.id)),
	...
	m.reply = async (text, options = {}) => {
		if (typeof text === 'string') {
			return await sockk.sendMessage(m.from, { text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		} else if (typeof text === 'object' && typeof text !== 'string') {
			return sockk.sendMessage(m.from, { ...text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		}
	};
